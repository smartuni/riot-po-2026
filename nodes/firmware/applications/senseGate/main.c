...
#endif
			   .limit_switch_confs = {{.pin = REED_0_PIN_0, .pull_conf = GPIO_IN_PU, .closed_level = false}},
			   },
};
...
int main(void) {
	...
	_LOGDBG("initial gate state: %s\n",
			initial_gate_state == GATE_STATE_CLOSED ? "CLOSED" : (initial_gate_state == GATE_STATE_OPEN ? "OPEN" : "INVALID"));
	...
	//init the payload
	health_monitor_payload_t battery_payload;
	health_monitor_payload_t shock_payload;
	while (1) {
		...
		//get battery info
		battery_payload.header = BATTERY_LOW;
		battery_payload.body = 3700;

		//get shock info
		shock_payload.header = SHOCK_STATUS;
		shock_payload.body = 1;

		//serialize the payload
		uint8_t buffer[64];
		size_t original_buff_size = sizeof(buffer);
		size_t buff_size = original_buff_size;
		health_monitor_serialize_record_no_sig(&battery_payload, buffer, &buff_size);

		LOG_DEBUG("[main.c:%d] Serialized health monitor payload, size: %d bytes:\n", __LINE__, buff_size);
		...
		// if (status == 0) {
		// 	LOG_DEBUG("[main.c:%d] Sent health monitor payload via LoRaWAN\n", __LINE__);
		// } else {
		// 	LOG_DEBUG("[main.c:%d] Failed to send health monitor payload via LoRaWAN: lorawan is not initialized yet\n", __LINE__);
		// }

		memset(buffer, 0, sizeof(buffer));
		buff_size = original_buff_size;
		health_monitor_serialize_record_no_sig(&shock_payload, buffer, &buff_size);
		LOG_DEBUG("[main.c:%d] Serialized health monitor payload, size: %d bytes:\n", __LINE__, buff_size);
		for (size_t i = 0; i < buff_size; i++) {
			LOG_DEBUG("%02X ", buffer[i]);
		}
